...
export default {
	...
	computed: {
		...
		isAdmin() {
			return (
				this.$store.state.user.userInfoObj.roleNames &&
				this.$store.state.user.userInfoObj.roleNames.includes('超级管理员')
			)
		}
	},
	watch: {
		...
		activeIndex(newValue) {
			const menuName = this.myFileMenuMap[Number(newValue)] || (newValue === 'admin' ? '后台管理' : '')
			document.title = `${menuName} - ${this.$config.siteName}`
			this.isDrawer = false
		},
	...
	mounted() {
		const menuName = this.myFileMenuMap[Number(this.activeIndex)] || (this.activeIndex === 'admin' ? '后台管理' : '')
		document.title = `${menuName} - ${this.$config.siteName}`
	}
}
</script>
