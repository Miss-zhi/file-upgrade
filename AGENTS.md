...
> 本文件是 AI Agent 在本项目中工作的权威指南。所有代码生成、修改、审查必须遵守以下约定。
...
| 层 | 技术 | 版本 |
|---|---|---|
| 后端框架 | Spring Boot (qiwenshare parent) | 2.x / Java 8 |
| ORM | MyBatis-Plus + JPA/Hibernate（双 ORM） | MyBatis-Plus 3.x |
| 安全 | Spring Security + JWT（prime-jwt） | — |
| 搜索 | Elasticsearch | 8.1.2 |
| 缓存 | Redis (Lettuce) | — |
| 存储抽象 | UFOP（自研，支持 Local/Aliyun OSS/FastDFS/MinIO/Qiniu） | — |
| 文档编辑 | OnlyOffice Document Server | — |
| 前端框架 | Vue 2 + Element UI 2.x | Vue 2.6 / Element 2.15 |
| 构建工具 | Vue CLI 4 | — |
| 状态管理 | Vuex 3 | — |
| 样式 | Stylus | — |
...
│       ├── file/                ← 主业务模块
│       └── ufop/                ← 存储抽象模块
├── qiwen-file-web/              ← 前端 Vue 模块
│   ├── AGENTS.md                ← 前端模块级约定
│   ├── package.json
│   └── src/
├── documents/                   ← 项目文档
├── docker-compose.yml           ← OnlyOffice 容器编排
...
```
View(页面组件) → Component(可复用组件) → request/(API调用) → store/(状态管理)
```

- 页面组件在 `views/`，可复用组件在 `components/`
- API 调用按领域拆分到 `request/` 下的独立文件
- Vuex 模块在 `store/module/`
- 弹窗和浮层使用命令式服务模式（`Vue.extend()` + Promise），挂载在 `this.$openDialog.*` 和 `this.$openBox.*`

### 禁止事项

- **不要**修改 `application.properties` 中的数据库连接、Redis 连接、JWT 密钥等安全配置
- **不要**修改 `SecurityConfig.java` 中的 URL 白名单和权限规则，除非 Spec 明确要求
- **不要**引入 pom.xml / package.json 中不存在的新依赖，必须先说明理由
- **不要**删除或重命名任何已有的 API 端点（前端依赖这些路径）
- **不要**修改 UFOP 存储抽象层的接口定义，除非同步更新所有 5 种存储实现
- **不要**在 Controller 中直接操作数据库，必须通过 Service 层
- **不要**在前端组件中直接操作 DOM，使用 Vue 的 ref 或指令
...
- **日期处理**：当前使用 `DateUtil.getCurrentTime()` 返回 varchar 格式，保持一致
...
- **Lombok**：Entity 和 DTO/VO 统一使用 `@Data`，Service 注入用 `@Autowired`
...
- **引号**：单引号
- **分号**：不使用行尾分号
- **路径别名**：`@/` → `src/`，`_c/` → `components/`，`_v/` → `views/`，`_a/` → `assets/`，`_r/` → `request/`
- **组件命名**：PascalCase 文件名，kebab-case 模板引用
- **样式**：所有组件使用 `<style lang="stylus" scoped>`，Element UI 覆盖用 `>>>` 或 `::v-deep`
- **API 调用**：在 `request/` 目录定义，组件中 import 使用，不在组件中直接写 axios 调用
...
- 前端：组件测试使用 Vitest + Vue Test Utils（升级后），当前至少保证 `npm run lint` 和 `npm run build` 通过
...
1. ESLint 检查：`npx vue-cli-service lint --no-fix`
2. 生产构建：`npx vue-cli-service build`
