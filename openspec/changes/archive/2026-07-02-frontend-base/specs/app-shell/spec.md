...
构建奇文网盘 Vue 3 前端的三段式布局框架，1:1 还原旧项目的 Header / AsideMenu / Footer 布局，包括精确尺寸、交互行为和条件显隐逻辑。
...
- **FileView 页面**：el-aside 渲染 AppAside（侧边栏 210px），el-main 渲染子路由
...
- **折叠/展开**：右侧有 12px 宽 100px 高的切换条，点击切换 el-menu collapse 状态
...
- **移动端（≤768px）**：渲染为 `el-drawer`（size 210px，`direction="ltr"` 即从左向右打开，Element Plus 的 ltr 等价于旧项目 Element UI 2 的 ltr）
- **路由激活高亮**：当前 fileType 对应的菜单项高亮
...
- **版权信息**：通过 API `getSystemParams()` 获取 copyright 配置，显示在 Logo 下方
...
// AppLayout.vue
// Props: none
// Computed: showHeader (from route meta), showFooter (from route meta)
// Template: AppHeader + el-container(el-aside(AppAside) + el-main(router-view)) + AppFooter

// AppHeader.vue
// Props: none
// Uses: useAuthStore, useRouter, useCommonStore (screenWidth)
// Methods: handleLogout, handleChangePassword, navigateTo(path)

// AppAside.vue
// Props: none
// Uses: useSideMenuStore, useAuthStore, useRoute, useRouter
// State: isCollapsed (localStorage persisted)
// Methods: toggleCollapse, handleMenuClick(fileType), fetchStorage

// AppFooter.vue
// Props: none
// State: copyrightInfo (from API)
```

## Dependencies

- Element Plus: el-container, el-aside, el-main, el-menu, el-dropdown, el-drawer, el-progress
- Pinia stores: auth, sideMenu, common
- API: home.getSystemParams()
- **跨 Capability 依赖**：AppAside 依赖 sideMenu store 的 `fetchStorage()` action（app-store），该 action 依赖 file.getStorage() API（app-api）。执行顺序：app-api → app-store → app-shell
