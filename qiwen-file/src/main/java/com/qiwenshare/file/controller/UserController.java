...
import com.qiwenshare.file.dto.user.UpdatePasswordDTO;
...
public class UserController {
    ...
    public RestResult<UserLoginVo> userLogin(
        ...
        if (result == null) {
            return RestResult.<UserLoginVo>fail().message("手机号或密码错误！");
        }
        ...
        try {
            ...
            log.info("登录失败：{}", e);
            return RestResult.<UserLoginVo>fail().message("创建token失败！");
        }
        ...
        if (sessionUserBean.getAvailable() != null && sessionUserBean.getAvailable() == 0) {
            return RestResult.<UserLoginVo>fail().message("用户已被禁用");
        }
        UserLoginVo userLoginVo = new UserLoginVo();
        BeanUtil.copyProperties(sessionUserBean, userLoginVo);
        java.util.List<com.qiwenshare.file.domain.user.Role> roles = userService.selectRoleListByUserId(sessionUserBean.getUserId());
        java.util.List<String> roleNames = new java.util.ArrayList<>();
        for (com.qiwenshare.file.domain.user.Role role : roles) {
    ...
    public RestResult<UserLoginVo> checkUserLoginInfo(@RequestHeader("token") String token) {
        ...
        if (StringUtils.isNotEmpty(userId)) {
            ...
            java.util.List<com.qiwenshare.file.domain.user.Role> roles = userService.selectRoleListByUserId(userId);
            java.util.List<String> roleNames = new java.util.ArrayList<>();
            for (com.qiwenshare.file.domain.user.Role role : roles) {
                roleNames.add(role.getRoleName());
            }
            ...
            return RestResult.<UserLoginVo>success().data(userLoginVo);

        } else {
            return RestResult.<UserLoginVo>fail().message("用户暂未登录");
        }
    ...
    public RestResult<Boolean> checkWxAuth() {
        ...
        if (sessionUserBean != null && !"anonymousUser".equals(sessionUserBean.getUsername())) {
            ...
            if (StringUtils.isEmpty(user.getWxOpenId())) {
                return RestResult.<Boolean>success().data(false);
            } else {
                return RestResult.<Boolean>success().data(true);
            }

        } else {
            return RestResult.<Boolean>fail().message("用户暂未登录");
        }
    ...
    @ResponseBody
    public RestResult<String> updatePassword(@Valid @RequestBody UpdatePasswordDTO updatePasswordDTO) {
        JwtUser sessionUserBean = SessionUtil.getSession();
        if (sessionUserBean == null) {
            return RestResult.fail().message("用户暂未登录");
        }
        return userService.updatePassword(sessionUserBean.getUserId(), updatePasswordDTO.getOldPassword(), updatePasswordDTO.getNewPassword());
    }
