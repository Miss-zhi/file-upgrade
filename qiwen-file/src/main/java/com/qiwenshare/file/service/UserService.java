...
public class UserService extends ServiceImpl<UserMapper, UserBean> implements IUserService, UserDetailsService {
    ...
    @Override
    public RestResult<String> updatePassword(String userId, String oldPassword, String newPassword) {
        UserBean userBean = userMapper.selectById(userId);
        if (userBean == null) {
            return RestResult.fail().message("用户不存在");
        }
        String oldHashPassword = HashUtils.hashHex("MD5", oldPassword, userBean.getSalt(), 1024);
        if (!oldHashPassword.equals(userBean.getPassword())) {
            return RestResult.fail().message("旧密码错误");
        }

        String newSalt = PasswordUtil.getSaltValue();
        String newHashPassword = HashUtils.hashHex("MD5", newPassword, newSalt, 1024);
        userBean.setSalt(newSalt);
        userBean.setPassword(newHashPassword);
        userMapper.updateById(userBean);
        return RestResult.success().message("密码修改成功");
    }
